...
class StreamlitBeerGameApp:
    ...
    def render_sidebar(self) -> Dict[str, Any]:
        ...
        with st.sidebar.expander("📊 Order Quantity Limit", expanded=False):
            ...
            retailer_no_constraints = st.checkbox(
                ...
                value=False,
                key="retailer_no_constraints",
                help="Retailer will NOT see any order limit/constraint language in its prompts."
            )
            wholesaler_no_constraints = st.checkbox(
                "Wholesaler — no constraints",
                value=False,
                key="wholesaler_no_constraints",
                help="Wholesaler will NOT see any order limit/constraint language in its prompts."
            )
            distributor_no_constraints = st.checkbox(
                "Distributor — no constraints",
                value=False,
                key="distributor_no_constraints",
                help="Distributor will NOT see any order limit/constraint language in its prompts."
            )
            manufacturer_no_constraints = st.checkbox(
                "Manufacturer — no constraints",
                value=False,
                key="manufacturer_no_constraints",
                help="Manufacturer will NOT see any order limit/constraint language in its prompts."
            )

            all_no_constraints = all([
                retailer_no_constraints, wholesaler_no_constraints,
                distributor_no_constraints, manufacturer_no_constraints
            ])
            any_no_constraints = any([
                retailer_no_constraints, wholesaler_no_constraints,
                distributor_no_constraints, manufacturer_no_constraints
            ])

            if any_no_constraints:
                unconstrained = [r for r, v in [
                    ("Retailer", retailer_no_constraints),
                    ("Wholesaler", wholesaler_no_constraints),
                    ("Distributor", distributor_no_constraints),
                    ("Manufacturer", manufacturer_no_constraints),
                ] if v]
                st.info(f"No-constraint roles: {', '.join(unconstrained)}. Constraint clauses will be removed from their prompts.")

            if all_no_constraints:
                enable_order_limits = False
                enable_discrete_points = False
                enable_adaptive_limits = False
            else:
                enable_order_limits = st.checkbox(
                    "Enable Min/Max Order Quantity Limit",
                    value=True,
                    help="Set min and max Order Quantity range limits for each Role (skipped for no-constraint roles)"
                )
            ...
            if enable_order_limits or enable_discrete_points:
                # Retailer
                if not retailer_no_constraints:
                ...
                # Wholesaler
                if not wholesaler_no_constraints:
                ...
                # Distributor
                if not distributor_no_constraints:
                ...
                # Manufacturer
                if not manufacturer_no_constraints:
                ...
                if not enable_discrete_points and not all_no_constraints:
                    lines = []
                    if not retailer_no_constraints:
                        lines.append(f"• Retailer: [{retailer_min_order}, {retailer_max_order}]")
                    if not wholesaler_no_constraints:
                        lines.append(f"• Wholesaler: [{wholesaler_min_order}, {wholesaler_max_order}]")
                    if not distributor_no_constraints:
                        lines.append(f"• Distributor: [{distributor_min_order}, {distributor_max_order}]")
                    if not manufacturer_no_constraints:
                        lines.append(f"• Manufacturer: [{manufacturer_min_order}, {manufacturer_max_order}]")
                    if lines:
                        st.success(
                    ...
                    lines = []
                    if retailer_points:
                        lines.append(f"• Retailer: {retailer_points}")
                    if wholesaler_points:
                        lines.append(f"• Wholesaler: {wholesaler_points}")
                    if distributor_points:
                        lines.append(f"• Distributor: {distributor_points}")
                    if manufacturer_points:
                        lines.append(f"• Manufacturer: {manufacturer_points}")
                    if lines:
                        st.success(
        ...
        config = {
            ...
            # Order Quantity Limit Config
            "retailer_no_constraints": retailer_no_constraints,
            "wholesaler_no_constraints": wholesaler_no_constraints,
            "distributor_no_constraints": distributor_no_constraints,
            "manufacturer_no_constraints": manufacturer_no_constraints,
        ...
        # Add Order Quantity Limit Parameters (per-role, skipping no-constraint roles)
        if enable_order_limits and not enable_discrete_points:
            limits_update = {}
            if not retailer_no_constraints:
                limits_update["retailer_min_order"] = retailer_min_order
                limits_update["retailer_max_order"] = retailer_max_order
            if not wholesaler_no_constraints:
                limits_update["wholesaler_min_order"] = wholesaler_min_order
                limits_update["wholesaler_max_order"] = wholesaler_max_order
            if not distributor_no_constraints:
                limits_update["distributor_min_order"] = distributor_min_order
                limits_update["distributor_max_order"] = distributor_max_order
            if not manufacturer_no_constraints:
                limits_update["manufacturer_min_order"] = manufacturer_min_order
                limits_update["manufacturer_max_order"] = manufacturer_max_order
            if limits_update:
                config.update(limits_update)
        ...
        if enable_discrete_points:
            ...
            if not retailer_no_constraints:
                retailer_points = [int(x.strip()) for x in retailer_discrete_points.split(',') if x.strip().isdigit() and int(x.strip()) > 0]
                discrete_update["retailer_discrete_points"] = retailer_points
            if not wholesaler_no_constraints:
                wholesaler_points = [int(x.strip()) for x in wholesaler_discrete_points.split(',') if x.strip().isdigit() and int(x.strip()) > 0]
                discrete_update["wholesaler_discrete_points"] = wholesaler_points
            if not distributor_no_constraints:
                distributor_points = [int(x.strip()) for x in distributor_discrete_points.split(',') if x.strip().isdigit() and int(x.strip()) > 0]
                discrete_update["distributor_discrete_points"] = distributor_points
            if not manufacturer_no_constraints:
                manufacturer_points = [int(x.strip()) for x in manufacturer_discrete_points.split(',') if x.strip().isdigit() and int(x.strip()) > 0]
                discrete_update["manufacturer_discrete_points"] = manufacturer_points
            if discrete_update:
                config.update(discrete_update)
    ...
    def create_game_config(self, ui_config: Dict[str, Any]):
        ...
        # Apply per-role no_decision_constraints flags
        for role in ["retailer", "wholesaler", "distributor", "manufacturer"]:
            role_config = getattr(config, role)
        ...
        for role in ["retailer", "wholesaler", "distributor", "manufacturer"]:
            role_config = getattr(config, role)
            no_const = " (no constraints)" if role_config.no_decision_constraints else ""
            print(f"   {role}: [{role_config.min_order_quantity}, {role_config.max_order_quantity}]{no_const}")
    ...
    def plot_inventory_levels_combined(self, result):
        """Plot all 4 roles' net inventory + customer demand on a single combined chart"""
        fig = go.Figure()
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color=self.status_colors['demand'], width=3, dash='dot'),
            marker=dict(size=8, symbol='diamond'),
        ))

        # Net inventory per role
        roles = ['retailer', 'wholesaler', 'distributor', 'manufacturer']
        ...
        for role, name in zip(roles, role_names):
            ...
            for round_data in result.round_history:
                if role in round_data['agents']:
                    ...
                    inv_value = end_state.get('inventory', 0)
                    backorder_value = end_state.get('backorder', 0)
                    net_inventory = inv_value - backorder_value
                    inventory.append(net_inventory)
                    if net_inventory < 0:
                        hover_text.append(f"Week {round_data['round']} | {name}: <b>Backorder {abs(net_inventory)}</b>")
                    elif net_inventory == 0:
                        hover_text.append(f"Week {round_data['round']} | {name}: <b>No Inventory</b>")
                    else:
                        hover_text.append(f"Week {round_data['round']} | {name}: {net_inventory} units")

            fig.add_trace(go.Scatter(
                x=weeks, y=inventory,
                mode='lines+markers',
                name=f'{name} Inventory',
                line=dict(color=self.role_colors[role], width=2),
                marker=dict(size=6),
                hovertext=hover_text,
                hoverinfo='text',
            ))
        ...
        fig.update_layout(
            title='Supply Chain Inventory Levels & Customer Demand (Combined View)',
            xaxis_title='Week',
