...
class BaseAgent(ABC):
    ...
    def receive_demand(self, demand: int):
        ...
        if len(self.state.demand_history) > 10:
    ...
    def get_decision_context(self) -> Dict[str, Any]:
        ...
        context = {
            ...
            'recent_orders': self.state.orders_history[-5:] if self.state.orders_history else [],
            'recent_inventory': self.state.inventory_history[-5:] if self.state.inventory_history else [],
            'recent_costs': self.state.cost_history[-5:] if self.state.cost_history else [],
            'demand_history': self.state.demand_history[-10:] if self.state.demand_history else [],
            'shipment_history': self.state.shipment_history[-5:] if self.state.shipment_history else [],
